...
export default function IntroScreen() {
  const [phase, setPhase] = useState<'converge' | 'hold' | 'reveal' | 'done'>('converge');
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
    // Only show once per session
    const hasSeenIntro = sessionStorage.getItem('tedx-intro-seen');
    if (hasSeenIntro) {
      setPhase('done');
      return;
    }
    ...
    const t3 = setTimeout(() => {
      setPhase('done');
      sessionStorage.setItem('tedx-intro-seen', 'true');
    }, 4200);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
