...
export default function FAQ() {
  ...
  return (
    <motion.div
      ...
      className="min-h-screen bg-[#050507] text-white relative overflow-hidden"
    >
      {/* Meteor Background */}
      ...
      <div className="relative z-10 px-6 md:px-16 max-w-screen-2xl mx-auto pt-40 pb-32">
        <header className="mb-24 flex flex-col items-center text-center">
          ...
          >
            <span className="font-typewriter text-[10px] text-[#E02229] tracking-[1em] uppercase">
              Information Center
            </span>
            <h1 className="text-7xl md:text-[8vw] font-title font-black tracking-tighter uppercase leading-[0.8] text-white flex flex-col items-center">
              <span>The</span>
              <span className="italic font-editorial lowercase text-[#E02229]">Essentials.</span>
            </h1>
          </motion.div>
        </header>
                ...
                <button
                  ...
                  className={`w-full text-left p-5 rounded-2xl flex items-center justify-between transition-all duration-500 group border ${
                    activeCategory === cat.category
                      ? 'bg-[#E02229]/10 border-[#E02229]/40 text-white shadow-lg shadow-[#E02229]/10'
                      : 'bg-white/[0.03] border-white/10 text-white/50 hover:bg-white/[0.06] hover:text-white'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-xl transition-colors ${activeCategory === cat.category ? 'bg-[#E02229]/20 text-[#E02229]' : 'bg-white/5 text-white/40'}`}>
                      {cat.icon}
                    </div>
                  ...
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${activeCategory === cat.category ? 'border-[#E02229]/60 rotate-45' : 'border-white/20'}`}>
                    <Plus size={10} />
                  </div>
                </button>
              ))}

              <div className="mt-10 p-6 bg-white/[0.04] rounded-[1.5rem] border border-white/10 space-y-4">
                ...
                <a href="mailto:info@tedxalmuntazir.com" className="inline-block font-sans font-bold text-xs uppercase tracking-widest text-[#E02229] hover:underline">
                  Contact Organizers
                </a>
              </div>
            </div>
            ...
            {FAQ_DATA.map((cat) => (
              ...
              >
                <div className="flex items-center gap-6">
                  <div className="w-10 h-[1px] bg-[#E02229]" />
                  <span className="font-typewriter text-[10px] text-[#E02229] tracking-[0.5em] uppercase">{cat.category}</span>
                </div>
                ...
                <div className="space-y-4">
                  {cat.questions.map((faq, i) => (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className="group border border-white/10 rounded-2xl bg-white/[0.03] hover:border-[#E02229]/30 hover:bg-white/[0.05] transition-all duration-500 overflow-hidden"
                    >
                      <button
                      ...
                      >
                        ...
                        <div className={`shrink-0 w-8 h-8 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 ${openQuestion === `${cat.category}-${i}` ? 'rotate-45 border-[#E02229]/50 bg-[#E02229]/10' : ''}`}>
                          <Plus size={14} className={openQuestion === `${cat.category}-${i}` ? 'text-[#E02229]' : 'text-white/40'} />
                        </div>
                      </button>
                      <AnimatePresence>
