        ...
        <header className="mb-32 flex flex-col items-center text-center mt-12 md:mt-24 overflow-hidden">
          ...
          >
            ...
            <h1 className="text-[16vw] md:text-[14vw] font-title font-black tracking-tighter leading-[0.8] uppercase text-white flex flex-col items-center">
              <span>The</span>
              <span className="italic font-editorial lowercase text-brand-secondary">
                Architects.
              </span>
            </h1>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
          {TEAM_GROUPS.map((group) => (
            <motion.div
              ...
              className="space-y-6"
            >
              <h4 className="font-title text-3xl uppercase text-white border-b border-white/30 pb-4 flex justify-between items-baseline">
                {group.category}
                <span className="font-typewriter text-[9px] text-brand-secondary opacity-60 tracking-widest">
                ...
                {group.members.map((m, mIdx) => (
                  ...
                  >
                    <div className="flex gap-2 items-center text-white">
                      <span
                        className={`font-sans text-lg ${m.lead ? "font-bold" : ""}`}
                      >
                        {m.name}
                      </span>
                      {m.lead && (
                    ...
                    <span className="font-typewriter text-[10px] text-white/60 shrink-0">
                      {m.year ? `Year ${m.year}` : ""}
                    </span>
                  </motion.div>
                ))}
