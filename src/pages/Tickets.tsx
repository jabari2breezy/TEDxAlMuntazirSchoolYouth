...
function StarDot({ s, scrollProgress }: { s: any; scrollProgress?: any }) {
  const parallaxY = scrollProgress
    ? useTransform(
        scrollProgress,
        [0, 1],
        [s.parallaxSpeed * 50, -s.parallaxSpeed * 50],
      )
    : undefined;
...
export default function Tickets() {
  ...
  // Ticket 3D Transforms - slower, more subtle rotation
  ...
  const xMovementDesktop = useTransform(
    scrollYProgress,
    [0, 0.25, 0.35, 0.65, 0.75, 1],
    [0, 0, 180, 180, 0, 0],
  );

  const yMovementMobile = useTransform(
    scrollYProgress,
    [0, 0.25, 0.35, 0.65, 0.75, 1],
    ["0%", "0%", "35%", "35%", "0%", "0%"],
  );
  ...
  const centerContentOpacity = useTransform(
    scrollYProgress,
    [0.14, 0.24, 0.58, 0.68],
    [0, 1, 1, 0],
  );

  const detailsYDesktop = useTransform(
    scrollYProgress,
    [0.14, 0.24, 0.58, 0.68],
    [50, 0, 0, -50],
  );
  const detailsYMobile = useTransform(
    scrollYProgress,
    [0.14, 0.24, 0.58, 0.68],
    [-40, 0, 0, -40],
  );
  ...
  return (
    <div className="bg-[#050507] text-white">
      {/* Noise overlay */}
      <div
        className="fixed inset-0 opacity-[0.12] pointer-events-none z-50 mix-blend-overlay"
        style={{
          backgroundImage:
      ...
      <div ref={containerRef} className="h-[180vh] relative">
        ...
        >
          ...
          <div className="absolute inset-0 z-0 pointer-events-none">
            <div
              className="absolute top-0 left-0 w-[60vw] h-[60vh] rounded-full opacity-10"
              style={{
                background:
                  "radial-gradient(circle, rgba(0,109,56,0.4) 0%, transparent 70%)",
                transform: "translate(-30%, -30%)",
              }}
            />
            <div
              className="absolute bottom-0 right-0 w-[50vw] h-[50vh] rounded-full opacity-10"
              style={{
                background:
                  "radial-gradient(circle, rgba(0,8,57,0.5) 0%, transparent 70%)",
                transform: "translate(30%, 30%)",
              }}
            />
          </div>
            ...
            <>
              ...
              >
                <div className="space-y-3">
                  <div>
                    <span className="font-typewriter text-[9px] uppercase tracking-[0.5em] text-brand-secondary">
                      The Inclusion
                    </span>
                    <h3 className="text-3xl font-title font-black uppercase tracking-tighter leading-none mt-1">
                      Event Details
                    </h3>
                    <p className="font-editorial text-base italic text-white/60 mt-1">
              ...
              <motion.div
                ...
                onClick={() => {
                  if (isPhase3)
                    window.open(
                      "https://tukiio.com/event/tedxalmuntazirschoolsyouth",
                      "_blank",
                    );
                }}
              >
              ...
              >
                <div className="space-y-8">
                  <div className="space-y-2">
                    ...
                    <h3 className="text-6xl font-title font-black uppercase tracking-tighter leading-none">
                      Event Details
                    </h3>
                    <p className="font-editorial text-xl italic text-white/60">
                      June 14, 2026. Al Muntazir Nursery Campus.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-white/60 leading-relaxed font-typewriter">
                      Proceeding to checkout will redirect you to the Tukiio
                      portal. Please have your M-Pesa or card details ready.
                    </p>
                  </div>
                </div>
              </motion.div>
...
function TicketCard({ isPhase3 }: { isPhase3: boolean }) {
  return (
    <div
      className={`relative rounded-[2.5rem] overflow-hidden bg-[#f7f4ee] shadow-[0_50px_100px_rgba(0,0,0,0.5)] border border-white/10 transition-all duration-500 ${isPhase3 ? "ring-4 ring-brand-secondary ring-offset-4 ring-offset-[#050507]" : ""}`}
    >
      {/* Ticket Top */}
      <div className="bg-[#000839] px-8 md:px-14 pt-10 md:pt-16 pb-12 md:pb-14 relative overflow-hidden">
        ...
        <div className="mt-6 md:mt-8 grid grid-cols-2 gap-4 relative z-10">
          <div>
            <span className="font-typewriter text-[8px] uppercase tracking-widest text-white/20 block mb-1">
              Access
            </span>
            <p className="font-title font-bold text-white text-sm md:text-base uppercase">
              Full Event
            </p>
          </div>
          <div>
      ...
      {/* Ticket Bottom */}
      <div
        className={`bg-[#f7f4ee] px-8 py-8 md:py-12 flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-500 ${isPhase3 ? "bg-brand-secondary text-white" : "text-[#000839]"}`}
      >
        <AnimatePresence mode="wait">
          {!isPhase3 ? (
            ...
            >
              <span className="font-typewriter text-[10px] uppercase tracking-widest opacity-40">
                Admission Fee
              </span>
              <div className="flex items-baseline gap-2">
                <span className="font-typewriter text-xs uppercase opacity-40">
                  Tsh
                </span>
                <span className="font-title font-black text-4xl md:text-5xl tracking-tighter">
                  30,000
                </span>
              </div>
            </motion.div>
